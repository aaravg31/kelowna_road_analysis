...
plot_diff_map(centrality_compare, "closeness_diff", "Change in Closeness (3D – 2D)", limit_range = c(-0.1, 0.1))
plot_diff_map(centrality_compare, "betweenness_diff", "Change in Betweenness (3D – 2D)", limit_range = c(-0.05, 0.05))
plot_diff_map(centrality_compare, "pagerank_diff", "Change in PageRank (3D – 2D)", limit_range = c(-0.001, 0.001))
```
...
cat("Top Edges by Edge Betweenness (2D):\n")
print(top5_edges_2d)
cat("\nTop Edges by Edge Betweenness (3D):\n")
print(top5_edges_3d)
```
...
# Extract top 10 edge IDs for each
top5_edge_ids_2d <- as.character(top5_edges_2d$edge_id)
top5_edge_ids_3d <- as.character(top5_edges_3d$edge_id)


par(mfrow = c(1, 2), mar = c(1, 1, 3, 1))
plot_top_edges(g_2d, edges_with_bet, top5_edge_ids_2d, "Top 10 Edge Betweenness (2D)")
plot_top_edges(g_3d, edges_with_bet, top5_edge_ids_3d, "Top 10 Edge Betweenness (3D)")
```
## Mean Edge Betweeness by Road Class
```{r}
...
ggplot(bet_summary_long, aes(x = reorder(road_class, total_bet), y = total_bet, fill = model)) +
  ...
  coord_flip() +
  scale_fill_manual(values = c("2D" = "wheat1", "3D" = "slateblue2")) +
  labs(
...
# 8. Resiliance Analysis - Average Path length for Largest Connected Component after removing top k nodes and edges
```{r}
# ---------- Utilities ----------
...
# 9. Plotting Resiliance Analysis - Average Path length for Largest Connected Component after removing top k nodes and edges
```{r}
